...
window.addEventListener("load", function () {
    ...
    function inputHeader() {
        ...
        const headerSearchInputWrapper = document.querySelector('.header__search-input');
        const headerSearchInputWrapperMobile = document.querySelector('.header__search-input_mobile');
        const headerSearchInputWrapperMobileBtn = document.querySelector('.header__search-input_mobile button');
        const headerSearchInput = document.querySelector('.header__search-input input');
        const headerSearchInputMobile = document.querySelector('.header__search-input_mobile input');
        const headerSearchParent = document.querySelector('.header__search');
        const headerSearchPlaceholder = document.querySelector('.header__search-placeholder');
        const headerSearchResults = document.querySelector('.header__search-results');
        if(window.innerWidth > 767) {
            
            if(headerSearchParent) {
                headerSearchParent.addEventListener('click', (e) => {
                    headerSearchPlaceholder.style.opacity = '0';
                    headerSearchInputWrapper.classList.add('stable');
                });
            }
            ...
            if(headerSearchPlaceholder) {
                headerSearchPlaceholder.addEventListener('click',(e) => {
                    headerSearchPlaceholder.style.display = 'none';
                    headerSearchInputWrapperMobile.classList.add('active');
                    headerInner.classList.add('header_inner_searched');
                    if (headerSearchInputMobile.value.length > 0) {
                        headerSearchResults.classList.add('active');
                    }
                });
            }
            if(headerSearchInputWrapperMobileBtn) {
                headerSearchInputWrapperMobileBtn.addEventListener('click',(e) => {
                    headerSearchPlaceholder.style.display = 'flex';
                    headerSearchInputWrapperMobile.classList.remove('active');
                    headerInner.classList.remove('header_inner_searched');
                    headerSearchResults.classList.remove('active');
                });
            }
            if(headerSearchInputMobile) {
                headerSearchInputMobile.addEventListener('change', () => {
                    if (headerSearchInputMobile.value.length > 0) {
                        headerSearchResults.classList.add('active');
                    } else {
                        headerSearchResults.classList.remove('active');
        ...
        //     headerSearchInput.addEventListener('change', () => {
        //         if (headerSearchInput.value.length > 0) {
        //             headerSearchPlaceholder.classList.add('disabled');
        //             headerSearchInputWrapper.classList.add('stable');
        //             headerSearchResults.classList.add('active');
        //         } else {
        //             headerSearchResults.classList.remove('active');
        //             headerSearchInputWrapper.classList.remove('stable');
        //             headerSearchPlaceholder.classList.remove('disabled');
        //         }
        //     });
        // }
    ...
    //input promo
    function inputPromo() {
        const inputPromo = document.querySelector('.search__promo input');
        const submitPromo = document.querySelector('search__promo button');
        const resultsPromo = document.querySelector('.search__promo-results');

        if(inputPromo) {
            inputPromo.addEventListener('change', () => {
                if (inputPromo.value.length > 0) {
                    resultsPromo.classList.add('active');
                } else {
                    resultsPromo.classList.remove('active');
                }
            });
        }
    ...
    const editVacancyInformerHTML = () => {
        const vacancyInformer = document.getElementById('vacancy-informer');
        if (!vacancyInformer) return;

        for (const vacancy of vacancyInformer.querySelectorAll('.informer_row1')) {
            const linkElement = vacancy.querySelector('a');

            // create title span element
            const titleSpanElement = document.createElement('span')
            const titleTextContent = linkElement.textContent;
            const titleTextNode = document.createTextNode(titleTextContent);
            titleSpanElement.appendChild(titleTextNode);
            titleSpanElement.classList.add('informer_vac__title')

            // create salary span element
            const salarySpanElement = document.createElement('span')
            const salaryTextContent = linkElement.nextSibling.textContent;
            const salaryTextNode = document.createTextNode(salaryTextContent);
            salarySpanElement.appendChild(salaryTextNode);

            // change vacancy content
            linkElement.innerHTML = "";
            linkElement.appendChild(titleSpanElement);
            linkElement.appendChild(salarySpanElement);
            linkElement.classList.add('informer_vac__link')

            vacancy.innerHTML = "";
            vacancy.appendChild(linkElement);
        }
    }
    ...
    const showSpinnerNavigation = () => {
        ...
        function onWheel(e) {
            ...
            let new_value = Number(new_counter) * 10;
